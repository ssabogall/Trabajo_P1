...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        json_file_path = 'inventory/management/commands/rawmaterials.json' 
        ...
        for i in range(len(materials)):
            product = materials[i]
            exist = RawMaterial.objects.filter(name = product['name']).first() #Se asegura que la película no exista en la base de datos
            if not exist:
                try:              
            ...
            #     except:
            #         pass
        #self.stdout.write(self.style.SUCCESS(f'Successfully added {cont} materials to the database'))
